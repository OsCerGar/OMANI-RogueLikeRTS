Shader "Hidden/MyGPUGShader0"
{
	Properties {
			__UV_z__ ("UV z", Float) = 0.0
			XPeriod ("XPeriod", Float) = 2
			NoiseWidth ("NoiseWidth", Float) = 1
			YPeriod ("YPeriod", Float) = 3
			NoiseHeight ("NoiseHeight", Float) = 1
...
float __UV_z__;
			...
			{
				float NoiseNode17 = 1.0 * SmoothNoise(4.0 * float2(IN.texcoord.x, IN.texcoord.y));
float SimpleNode16 = (NoiseNode17 * TurbulencePower);
float SimpleNode12 = (IN.texcoord.y * YPeriod);
float SimpleNode13 = (SimpleNode12 / NoiseHeight);
float SimpleNode6 = (IN.texcoord.x * XPeriod);
float SimpleNode9 = (SimpleNode6 / NoiseWidth);
float SimpleNode14 = (SimpleNode9 + SimpleNode13);
float SimpleNode20 = (SimpleNode14 + SimpleNode16);
float SimpleNode34 = (SimpleNode20 * 3,14159.0);
float SimpleNode33 = (sin(SimpleNode34));
float SimpleNode32 = (abs(SimpleNode33));
float graphNoiseResult = SimpleNode32;
return float4(graphNoiseResult, graphNoiseResult, graphNoiseResult, 0);
